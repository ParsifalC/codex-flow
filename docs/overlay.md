...
新数据必须有 `publication` 才进入详情；旧的已结束记录仍可浏览。新字段优先，旧记录缺少
`turn_context.goal` 或 `result` 时，界面只读兼容使用已有 `summary_info.goal/conclusion`，并标记“历史兼容”；不会把旧值回写成新的发布字段。
...
### 2. Inspector（已完成轮次详情）
- **本轮目标与结果**：新记录分别读取 `turn_context.goal` 和本轮父 Agent 的 `result`；旧历史缺少新字段时兼容读取已有 `summary_info.goal/conclusion`，并标记“历史兼容”。两处都没有数据时显示“未记录”。父 Stop 发布之前不展示新轮次详情。
