...
同一个 Worker 可以跨轮复用。采集器按 Worker ID 与子执行轮次 ID 保存归属和消耗：跨轮执行分别计入各自的父轮次；同一父轮次内多次执行仍显示为一个 Worker，并汇总各次执行的 Token。重复结束事件不会重复累计，迟到事件仍归属原轮次。缺少归属证据时保留独立记录，避免覆盖其他轮次；复用 Worker 的整条线程累计消耗不能代替单次执行消耗。
