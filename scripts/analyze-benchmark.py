...
def load_jsonl(path: Path):
    ...
    for n, line in enumerate(path.read_text().splitlines(), 1):
        if not line.strip():
            continue
        row = json.loads(line)
        required = {"task_id","task_class","model","reasoning_effort","passed","input_tokens","cached_input_tokens","output_tokens","repair_cycles"}
...
def main():
    ...
    for row in rows:
        key = (row["task_class"], row["model"], row["reasoning_effort"])
    ...
    for (task_class, model, effort), items in sorted(grouped.items()):
        ...
        quality_ok = len(items) >= min_samples and pass_rate >= thresholds[task_class] and avg_repairs <= max_repairs
        summary.append({
    ...
    for task_class in ("routine", "complex", "critical"):
        ...
        if eligible:
            best = min(eligible, key=lambda x: (x["average_cost_usd"], EFFORT_RANK[x["reasoning_effort"]], x["model"]))
